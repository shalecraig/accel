...
void accel_destroy_gesture(accel_gesture **gesture, uint16_t dimensions) {
...
int accel_generate_gesture(accel_state *state, accel_gesture **gesture) {
    ...
    *gesture = (accel_gesture *)malloc(gesture_size);
    ...
    for (uint16_t i = 0; i < state->dimensions; ++i) {
...
// TODO: test generated value with PRECONDITION_VALID_STATE
...
void handle_recording_tick(accel_gesture *gesture, uint16_t dimensions) {
    if (gesture == NULL) {
        return;
    }
...
int accel_process_timer_tick(accel_state *state, int *accel_data) {
    ...
    for (int gesture_id = 0; gesture_id < state->state->num_gestures_saved; ++gesture_id) {
        ...
        for (uint16_t d = 0; d < state->dimensions && returned == 0; ++d) {
